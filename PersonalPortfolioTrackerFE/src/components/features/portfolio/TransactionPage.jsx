...
const getTransactionBadge = (type) => {
  switch (type?.toUpperCase()) {
    case "BUY":
      return (
        <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[11px] font-black rounded uppercase tracking-widest border border-emerald-100">
          BUY
        </span>
      );
    case "SELL":
      return (
        <span className="px-3 py-1 bg-rose-50 text-rose-600 text-[11px] font-black rounded uppercase tracking-widest border border-rose-100">
          SELL
        </span>
      );
    case "DIVIDEND_CASH":
      return (
        <span className="px-3 py-1 bg-blue-50 text-blue-600 text-[11px] font-black rounded uppercase tracking-widest border border-blue-100">
          CASH DIV
        </span>
      );
    case "DIVIDEND_TICKER":
      return (
        <span className="px-3 py-1 bg-purple-50 text-purple-600 text-[11px] font-black rounded uppercase tracking-widest border border-purple-100">
          TICKER DIV
        </span>
      );
    default:
      return (
        <span className="px-3 py-1 bg-gray-100 text-gray-600 text-[11px] font-black rounded uppercase tracking-widest">
          {type}
        </span>
      );
  }
...
export default function TransactionPage() {
  ...
  useEffect(() => {
    const fetchInitData = async () => {
      try {
        ...
        setAccounts(accData);
        if (accData.length > 0)
          setSelectedAccountId(accData[0].accountID || accData[0].accountId);

        const tTypes = typeRes.data?.result || typeRes.data || [];
        setTickerTypes(tTypes);
        if (tTypes.length > 0)
          setTickerSearchType(
            tTypes.find((t) => t.code.toUpperCase() === "STOCK")?.id ||
              tTypes[0].id,
          );
      } catch (err) {
        console.error(err);
  ...
  const currentAccount = accounts.find(
    (a) => (a.accountID || a.accountId) === selectedAccountId,
  );
  ...
  const totalBoughtStats = useMemo(() => {
    ...
    return {
      value: buys?.totalValue || 0,
      count: buys?.numberOfTransactions || 0,
    };
  }, [summary]);
  ...
  const totalSoldStats = useMemo(() => {
    ...
    return {
      value: sells?.totalValue || 0,
      count: sells?.numberOfTransactions || 0,
    };
  }, [summary]);
    ...
    try {
      if (editingTransaction) {
        ...
        await axiosInstance.put(
          `/Transactions/${editingTransaction.id || editingTransaction.ID}`,
          updatePayload,
        );
      ...
      setFormError(
        err.response?.data?.message || "Submit failed. Check server logs.",
      );
    } finally {
      setIsSaving(false);
    }
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="mb-4 flex justify-between items-end">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Record your BUY/SELL and dividend transactions.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
          <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col relative overflow-hidden">
            ...
            <span className="text-xs font-bold text-gray-800 mb-1 pl-2">
              Total Bought
            </span>
        ...
        <div className="flex flex-col xl:flex-row flex-wrap items-center gap-3 w-full mb-5">
          <div
            className="relative min-w-[140px] w-full xl:w-auto"
            ref={accDropdownRef}
          >
            <div
              onClick={() => setIsAccDropdownOpen(!isAccDropdownOpen)}
              className="flex items-center justify-between px-4 py-2 rounded-full border bg-white text-gray-800 font-semibold text-[13px] shadow-sm cursor-pointer border-pink-200 hover:border-pink-400 transition-colors"
            >
              {currentAccount ? (
              ...
              <div className="absolute top-full left-0 mt-2 w-full min-w-[200px] bg-white border border-gray-100 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.1)] py-2 z-30 overflow-hidden">
                {accounts.map((acc) => (
                  <div
                    ...
                    className={`flex items-center px-4 py-2 cursor-pointer text-[13px] ${selectedAccountId === (acc.accountID || acc.accountId) ? "bg-pink-50 text-pink-600 font-bold" : "hover:bg-gray-50 font-medium text-gray-700"}`}
                  >
                    {acc.accountName}
                  </div>
                ))}
          ...
          <div className="flex items-center w-full xl:w-auto bg-white border border-pink-200 rounded-full px-3 py-1.5 shadow-sm focus-within:border-pink-500 transition-colors">
            ...
            {(fromDate || toDate) && (
              <button
                onClick={clearDates}
                className="ml-1 text-rose-400 hover:text-rose-600 transition-colors flex items-center justify-center p-0.5 rounded-full hover:bg-rose-50"
                title="Clear dates"
              >
          ...
          <div className="ml-auto w-full xl:w-auto flex items-center justify-end gap-3 shrink-0">
            ...
            <button
              ...
              className="w-full xl:w-auto whitespace-nowrap px-5 py-2 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 text-white font-bold text-[12px] shadow-sm hover:-translate-y-0.5 transition-all"
            >
              + ADD TRANSACTION
            </button>
          </div>
        ...
        <div
          className={`space-y-3 transition-opacity duration-300 ${loading ? "opacity-40 pointer-events-none" : "opacity-100"}`}
        >
                      ...
                      {t.transactionType === "SELL" && (
                        <div>
                          ...
                          >
                            ...
                            >
                              ({formatPercent(t.realizedPnLRate || 0)})
                            </span>
                          </span>
                        </div>
                      )}
        ...
        {!loading && totalPages > 1 && (
          <div className="flex justify-center items-center gap-4 mt-5 mb-2">
            <button
              ...
              className="px-4 py-2 rounded-full bg-white border border-gray-200 text-[13px] font-semibold text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-all shadow-sm"
            >
              Previous
            </button>
        ...
        {detailTransaction && (
          ...
          >
            <div
              className="bg-white w-full max-w-xl rounded-[2rem] shadow-2xl relative animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh] overflow-hidden"
            ...
            >
              ...
              <div className="p-8 pb-4 shrink-0 bg-white border-b border-gray-100 z-10">
                <h2 className="text-3xl font-black text-gray-900 leading-none pr-8">
                  {detailTransaction.tickerSymbol}
                </h2>
                <p className="text-[11px] font-bold text-gray-400 mt-2 uppercase tracking-widest">
                  {detailTransaction.transactionType.replace("_", " ")}{" "}
                  TRANSACTION
                </p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-gray-50/30">
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm mb-5">
                  <div className="flex items-center gap-1.5 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">
                    <History size={12} /> Audit Track (Pre-Transaction)
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between text-[13px]">
                      <span className="text-gray-500 font-medium">
                ...
                {detailTransaction.note && detailTransaction.note !== "N/A" && (
                  <div>
                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 pl-1">
                      Trading Note
                    </div>
                    <div className="p-4 bg-yellow-50 rounded-2xl border border-yellow-100 text-sm text-yellow-800 italic">
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity">
            <div className="bg-white w-full max-w-lg rounded-[2rem] p-8 relative animate-in fade-in zoom-in duration-200 shadow-2xl">
              <button
                ...
                className="absolute top-6 right-6 text-gray-400 hover:text-gray-900 bg-gray-50 rounded-full p-2 transition-all z-10"
              >
                <X size={20} />
              </button>
              ...
              <form
                onSubmit={
                  !editingTransaction && !isReviewMode
                    ? (e) => {
                        e.preventDefault();
              ...
              >
                {!isReviewMode ? (
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="col-span-2 sm:col-span-1">
                        ...
                        >
                          ...
                          {accounts.map((acc) => (
                            <option
                              key={acc.accountID || acc.accountId}
                              value={acc.accountID || acc.accountId}
                            >
                              {acc.accountName}
                            </option>
                          ))}
                      ...
                      <div className="col-span-2 sm:col-span-1">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            !editingTransaction && setIsTickerModalOpen(true)
                          }
                    ...
                    {formError && (
                      <p className="text-rose-600 text-xs font-bold text-center pt-1">
                        {formError}
                      </p>
                    )}
                    ...
                    <div className="flex justify-end gap-3 pt-5 mt-2 border-t border-gray-100">
                      <button
                        type="button"
                        onClick={closeFormModal}
                        className="px-6 py-2.5 rounded-full border border-gray-200 text-gray-700 font-bold text-[13px] hover:bg-gray-50"
                      >
                        Cancel
                      </button>
                      <button
                        type="submit"
                        disabled={isSaving}
                        className="px-7 py-2.5 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 text-white font-black text-[13px] shadow-md hover:-translate-y-0.5 transition-all disabled:opacity-60"
                      >
                        {isSaving
                          ? "Saving..."
                          : editingTransaction
                  ...
                  <div className="animate-in fade-in zoom-in duration-200">
                    <h2 className="text-2xl font-black text-gray-900 mb-6 shrink-0 flex items-center gap-2">
                      <ShieldCheck className="text-emerald-500" /> Confirm
                      Details
                    </h2>
                    ...
                    <div className="bg-gray-50 p-5 rounded-2xl space-y-3 text-[13px] text-gray-700 border border-gray-100">
                      ...
                      {formData.note && (
                        <div className="flex flex-col gap-1 pt-1">
                          ...
                          <span className="italic text-gray-600 bg-white p-3 rounded-xl border border-gray-100">
                            {formData.note}
                          </span>
                        </div>
                      )}
                    ...
                    {formError && (
                      <p className="text-rose-600 text-xs font-bold text-center pt-3">
                        {formError}
                      </p>
                    )}
                    ...
                    <div className="flex justify-end gap-3 pt-5 mt-4 border-t border-gray-100">
                      <button
                        ...
                        className="px-6 py-2.5 rounded-full border border-gray-200 text-gray-700 font-bold text-[13px] hover:bg-gray-50 transition-colors"
                      >
                        Back to Edit
                      </button>
                      <button
                        type="submit"
                        disabled={isSaving}
                        className="px-7 py-2.5 rounded-full bg-gradient-to-r from-emerald-400 to-emerald-500 text-white font-black text-[13px] shadow-md hover:-translate-y-0.5 transition-all disabled:opacity-60"
                      >
                ...
                {isSearchingTickers ? (
                  ...
                  tickerSearchResults.map((t) => (
                    <div
                      ...
                      className="flex items-center gap-3 p-3 rounded-2xl border border-gray-100 hover:border-pink-400 hover:bg-pink-50 cursor-pointer group"
                    >
                      <div className="w-10 h-10 rounded-full bg-gray-50 flex justify-center items-center group-hover:bg-white">
                        {getRawIcon(t.tickerTypeName, 18)}
                      </div>
