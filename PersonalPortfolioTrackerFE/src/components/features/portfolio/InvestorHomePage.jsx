...
export default function Dashboard() {
  ...
  const shouldSpanFullWidth = (section) =>
    isSectionExpanded(section) && !allExpanded;
  ...
  const handleToggleAll = () => {
    ...
    setExpandedSections({
      accountAllocation: nextExpanded,
      tickerAllocation: nextExpanded,
      topPerformers: nextExpanded,
      recentTransactions: nextExpanded,
    });
  };
  ...
  const accountTotal = useMemo(
    () => accountList.reduce((s, x) => s + x.value, 0),
    [accountList],
  );
  const tickerTotal = useMemo(
    () => tickerList.reduce((s, x) => s + x.value, 0),
    [tickerList],
  );
  ...
  const accountChartData = {
    ...
    datasets: [
      {
        data: accountList.map((x) => x.value),
        backgroundColor: accountList.map((x) => x.color),
        hoverOffset: 6,
      },
    ],
  };
  ...
  const tickerChartData = {
    ...
    datasets: [
      {
        data: tickerList.map((x) => x.value),
        backgroundColor: tickerList.map((x) => x.color),
        hoverOffset: 6,
      },
    ],
  };
      ...
      plugins: {
        ...
        tooltip: {
          ...
          external: (context) => {
            ...
            tooltipEl.style.top =
              position.top + window.scrollY + tooltip.caretY - 8 + "px";
          },
          callbacks: {
            label: (ctx) => {
              ...
              const percent =
                total > 0 ? ((raw / total) * 100).toFixed(2) : "0.00";
  ...
  if (!data)
    return (
      <div className="p-10 text-center text-pink-500 font-bold animate-pulse">
        Loading dashboard...
      </div>
    );
  ...
  const ExpandButton = ({ section, title }) => {
    ...
    return (
      <button
        ...
        className="w-8 h-8 rounded-full border border-gray-200 bg-white
                 ...
                 hover:text-pink-500 hover:border-pink-200
                 hover:bg-pink-50 transition-all shrink-0"
      >
  ...
  const AllocationBlock = ({ section, title, items, total, chartData }) => {
    const expanded = isSectionExpanded(section);

    return (
      <div
        className={`bg-white rounded-2xl p-4 md:p-5 shadow-sm
                  border border-pink-50 flex flex-col h-full relative
      ...
      >
        <div className="flex items-center justify-between mb-3 shrink-0">
          ...
          {!isExporting ? (
            <ExpandButton section={section} title={title} />
          ) : (
            <div className="w-8" />
          )}
        </div>
        ...
        {items.length === 0 ? (
          ...
          <div
            className={`flex flex-col gap-5 items-center flex-1 ${
              expanded
          ...
          >
            <div
              ...
              transition-all duration-300 ${
                allExpanded
                  ? "w-48 h-48 xl:w-56 xl:h-56"
                  : expanded
                    ? "w-64 h-64 lg:w-72 lg:h-72"
                    : "w-48 h-48"
              }`}
            >
            ...
            <div
              className={`flex-1 min-w-[220px] w-full bg-white rounded-xl
                        border border-gray-100 p-2 ${
                          isExporting || expanded
                            ? "overflow-visible max-h-none h-auto"
                            : "max-h-[200px] overflow-y-auto custom-scrollbar pr-1"
                        }`}
            >
              {items.map((x, idx) => (
                <div
                  key={`${x.label}-${idx}`}
                  className={`grid grid-cols-[12px_minmax(0,1fr)_auto] gap-3
                items-center px-2 py-2.5 text-xs leading-5 ${
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div
                      className={`font-bold text-gray-800 leading-5 py-0.5 ${
                        isExporting
                    ...
                    <div className="text-gray-500 text-[10px] leading-4">
                      {vnd(x.value)}
                    </div>
                  </div>

                  <div className="font-extrabold text-gray-800 text-right whitespace-nowrap leading-5">
                    {hideAmounts
                      ? "**"
                      : formatPercent2(total > 0 ? (x.value / total) * 100 : 0)}
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="w-full max-w-6xl space-y-5">
        ...
        <div ref={headerRef} className="flex justify-between items-center mb-1">
          ...
          >
            <button
              ...
              className="h-10 px-3 sm:px-4 flex items-center justify-center gap-2
               rounded-full bg-white border border-gray-200 shadow-sm
               text-gray-600 hover:text-pink-500 hover:border-pink-200
          ...
          {[
            {
              ...
              badge: "Total",
              value: zone1?.portfolioValue || 0,
            },
            {
              title: "Cash Balance",
              badge: "Accounts",
              value: zone1?.cashBalance || 0,
            },
            {
              title: "Unrealized P&L",
              badge: "Open",
              value: zone1?.unrealizedPnL || 0,
              pnlRate: zone1?.unrealizedPnLRate || 0,
            },
            {
              title: "Realized P&L",
              badge: "Closed",
              value: zone1?.realizedPnL || 0,
            },
          ].map((c, i) => (
            <div
              key={i}
            ...
            >
              <div className="text-sm font-semibold text-gray-500 mb-2 flex items-center gap-2">
                {c.title}
                <span className="px-2 py-0.5 rounded-full bg-pink-50 text-pink-500 text-[10px] uppercase font-bold">
                  {c.badge}
                </span>
              </div>
              <div
                className={`text-2xl font-extrabold mb-1 ${c.title.includes("P&L") && c.value !== 0 ? getPnLColor(c.value) : "text-gray-900"}`}
              >
                {c.title.includes("Unrealized") && c.value > 0 ? "+" : ""}
                {vnd(c.value)}
              </div>
              {c.pnlRate != null ? (
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold mt-1 ${c.pnlRate >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
                >
                  {hideAmounts
                    ? "**"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div
            ...
            className={`h-full transition-all duration-300 ${
              shouldSpanFullWidth("accountAllocation") ? "lg:col-span-2" : ""
            }`}
          >
            <AllocationBlock
              section="accountAllocation"
          ...
          <div
            ref={allocTickerRef}
            className={`h-full transition-all duration-300 ${
              shouldSpanFullWidth("tickerAllocation") ? "lg:col-span-2" : ""
            }`}
          >
            <AllocationBlock
              section="tickerAllocation"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div
            ...
            className={`bg-white rounded-2xl p-4 md:p-5 shadow-sm
  border border-pink-50 flex flex-col h-full
  transition-all duration-300 ${
    shouldSpanFullWidth("topPerformers") ? "lg:col-span-2" : ""
  }`}
          >
            <div className="flex items-center justify-between mb-4 shrink-0">
              ...
              {!isExporting && (
                <ExpandButton section="topPerformers" title="Top Performers" />
              )}
            </div>
            {!topPerformers || topPerformers.length === 0 ? (
              ...
              <div
                className={`custom-scrollbar pr-2 flex-1 ${
                  isExporting || isSectionExpanded("topPerformers")
                    ? "overflow-visible max-h-none h-auto"
              ...
              >
                <table className="w-full text-sm text-left relative">
                  <thead
                    className={`text-xs text-gray-500 font-semibold bg-white z-10 ${
                      isSectionExpanded("topPerformers") || isExporting
                        ? ""
                        : "sticky top-0"
                    }`}
                  >
                    <tr>
                      <th className="pb-2 pt-1 border-b border-gray-100">
                        Ticker
                      </th>
                      <th className="pb-2 pt-1 border-b border-gray-100">
                        Account
                      ...
                      <th className="pb-2 pt-1 text-right border-b border-gray-100">
                        P&L
                      </th>
                    </tr>
                  </thead>
                    ...
                    {topPerformers.map((x, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2.5 text-gray-500 text-xs truncate max-w-[100px]">
                            {x.accountName}
                          </td>
                          <td className="py-2.5 text-gray-800 font-medium">
                            {isCrypto
                              ? usd(x.totalMarketValue)
                              : vnd(x.totalMarketValue)}
                          </td>
                          <td
                            className={`py-2.5 text-right font-bold ${x.unrealizedPnLRate >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                          >
                            {hideAmounts
                              ? "**"
                              : (x.unrealizedPnLRate >= 0 ? "+" : "") +
                                x.unrealizedPnLRate.toFixed(2) +
                                "%"}
                          </td>
                        </tr>
                      );
  ...
  border border-pink-50 flex flex-col h-full
  transition-all duration-300 ${
    shouldSpanFullWidth("recentTransactions") ? "lg:col-span-2" : ""
  }`}
          >
            <div className="flex items-start justify-between shrink-0 mb-4">
              <div>
                <h3 className="text-sm font-semibold text-gray-500 mb-1">
                  8 Recent BUY/SELL Transactions
                </h3>
            ...
            {!recentTransactions || recentTransactions.length === 0 ? (
              ...
              <div
                className={`custom-scrollbar pr-2 flex-1 ${
                  isExporting || isSectionExpanded("recentTransactions")
                    ? "overflow-visible max-h-none h-auto"
                    : "overflow-y-auto max-h-[320px]"
                }`}
              >
                        ...
                        >
                          <td className="py-2.5">
                            <div className="font-medium">
                              ...
                              - {t.accountName} : {t.symbol}
                            </div>
                            <div className="text-xs text-gray-400 mt-0.5">
                              {formatTradingDate(t.tradeDate)}
                            </div>
                          </td>
                          <td className="py-2.5 text-right">
                            <div className="text-gray-700 font-medium">
                              ...
                              {isCrypto
                                ...
                                : hideAmounts
                                  ? moneyMask
                                  : `${formatPriceVnd(t.price)} VND`}
                            </div>
                            <div
                              className={`text-xs font-bold mt-0.5 ${isInflow ? "text-emerald-600" : "text-rose-600"}`}
                            >
                              {isInflow ? "+" : "-"}
                              {isCrypto ? usd(absAmount) : vnd(absAmount)}
                            </div>
                          </td>
                        </tr>
                      );
